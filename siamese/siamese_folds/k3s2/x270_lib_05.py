...
def create_filepath_cat_from_htk_list(htk_list,real_dir):
    ...
    for line in lines:
        ...
        #EOL unis stule \r\n
        basen=basen.split('\r')[0] #basename
        basen=basen.split('\n')[0] #basename
        ...
        for testdir in real_dir:
            ...
            if os.path.isfile(f1):
                found=True
                total_found+=1
                # dir_idx=dir2idx(testdir) # same dix dictionanr from env_global ?
                # print ("found {} idx: {}".format(f1,dir_idx)) ; quit()
    ...
    dir_idx=0
    print ("Plese Synchronize cat/dir with index")
    for testdir in real_dir:
        print("dir/cat index : {} => {}",dir_idx,testdir)
        ++dir_idx
    return retlist
...
def folds_init_journal_train(ftrain, smaller_dir,categs):
    '''
    ...
    catdir=["",""]
    for i in range(2):
        catdir[i]="{}\\{}".format(smaller_dir,categs[i])
...
from env_global import categs
# p=Path("y:/fp_image")
# p=Path("y:/answer_sheet")
def dir2idx(dirname): #STIL ERROR
    p=Path(dirname)
    last_dir=p.parts[len(p.parts)-1]
    # print(last_dir)
    carr=np.array(categs)
    # print (carr.shape)
    idx=np.where(carr == last_dir)[0]
    # print (idx)
    return idx
