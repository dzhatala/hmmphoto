...
% return
	...
	[hcolor,widthcolor,channels]=size(A)
	r0_c0=A(1,1,:)
	r0_c1=A(2,1,:)
	r1_c0=A(1,2,:)
	
	% disp('hit any char!'); pause
	A=rgb2gray(A);
	...
	% col1_row0=A(2,1)
	% col0_row1=A(1,2)
	% block4_wxh=A(1:7,1:7)'
	block4_wxh=A(1:15,1:15)
...
% for rbig=1:1
	...
	% for irowblock=1:3
		...
		for ihorzblock=1:numBlock_horz
			if irowblock==1 && ihorzblock ==1
				irowblock,ihorzblock
			end
			...
			[coefs,adct2]=feature_hatala(imgblock); %dct2 inside funct.
...
disp("saving octave vars into gen_obs.mat"); save -binary tmp_genobs/mfccs.mat MFCCS;
...
if MARKER_LENGTH >0
	...
	fr_start=1;
	for irowblock=1:irowb_end
		disp (["padding row ... " num2str(irowblock) ]); 
...
% parmKind = 6 % MFCC six
[sampPeriod,parmKind]=mfcParams%in mfcParams.m
...
%test 
disp("saving octave vars into gen_obs.mat"); save -binary tmp_genobs/mfccs_padded.mat MFCCS;
